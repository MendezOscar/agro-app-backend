...
const form = ref<Omit<Input, 'id'>>({ name: '', kind: 0, unit: '', unitCost: 0 })
...
function reset() {
  form.value = { name: '', kind: 0, unit: '', unitCost: 0 }
...
function edit(i: Input) {
  ...
  form.value = { name: i.name, kind: i.kind, unit: i.unit, unitCost: i.unitCost }
}
...
<template>
  ...
  <div class="row">
    <div class="card" style="flex:2;min-width:360px">
      <table>
        <thead><tr><th>Nombre</th><th>Tipo</th><th>Unidad</th><th>Costo unit.</th><th></th></tr></thead>
        <tbody>
          ...
          <tr v-if="!inputs.length"><td colspan="5" class="muted">Sin insumos aún.</td></tr>
        </tbody>
      </table>
    </div>
